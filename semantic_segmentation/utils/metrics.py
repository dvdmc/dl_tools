...
def precision_from_conf_matrices(conf_matrices: List[torch.Tensor], ignore_index: Optional[int] = None) -> float:
...
def recall_from_conf_matrices(conf_matrices: List[torch.Tensor], ignore_index: Optional[int] = None) -> float:
    def conf_mat_masked(conf_mat):
        if ignore_index is None:
            return conf_mat
...
def f1_score_from_conf_matrices(conf_matrices: List[torch.Tensor], ignore_index: Optional[int] = None) -> float:
...
def mean_iou_from_conf_matrices(conf_matrices: List[torch.Tensor], ignore_index: Optional[int] = None) -> float:
    per_class_iou = per_class_iou_from_conf_matrices(conf_matrices, ignore_index=ignore_index)
...
def accuracy_from_conf_matrices(conf_matrices: List[torch.Tensor], ignore_index: Optional[int] = None) -> float:
    def conf_mat_masked(conf_mat):
        if ignore_index is None:
            return conf_mat
