...
class ASPP(nn.Module):
    def __init__(self, os, n_classes):
    ...
    def forward(self, x, low_level_features, input):
        x1 = self.aspp1(x)
        x2 = self.aspp2(x)
        x3 = self.aspp3(x)
        x4 = self.aspp4(x)
...
class DeepLabV3Model_MCD(nn.Module):
    def __init__(self, n_classes=21, nInputChannels=3, os=16, pretrained=False, _print=True):
        if _print:
        ...
        self.classifier = ASPP(os, n_classes)

        
    def forward(self, input):

        x, low_level_features = self.backbone(input)
        x = self.classifier(x, low_level_features, input)
...
if __name__ == "__main__":
    model = DeepLabV3Model_mcd(nInputChannels=3, n_classes=21, os=16, pretrained=True, _print=True)
    model.eval()
